...
document.addEventListener("DOMContentLoaded", () => {
  // Transactors pagination (already exists)
  const rowsTransactors = document.querySelectorAll("#transactorsTableBody tr");
  const perPage = 5;
  let currentPageTransactors = 1;
  const totalPagesTransactors = Math.ceil(rowsTransactors.length / perPage);
  const paginationContainer = document.getElementById("paginationControls");

  function renderTransactorsPage(page) {
    currentPageTransactors = page;
    rowsTransactors.forEach((row, index) => {
      row.style.display = (index >= (page-1)*perPage && index < page*perPage) ? "" : "none";
    });
    renderTransactorsPagination();
  }

  function renderTransactorsPagination() {
    paginationContainer.innerHTML = "";
    const prevBtn = document.createElement("button");
    prevBtn.textContent = "Prev";
    prevBtn.disabled = currentPageTransactors === 1;
    prevBtn.className = "px-3 py-1 border rounded bg-white text-blue-500 disabled:opacity-50";
    prevBtn.addEventListener("click", () => renderTransactorsPage(currentPageTransactors - 1));
    paginationContainer.appendChild(prevBtn);

    for (let i = 1; i <= totalPagesTransactors; i++) {
      const btn = document.createElement("button");
      btn.textContent = i;
      btn.className = `px-3 py-1 border rounded ${i === currentPageTransactors ? 'bg-blue-500 text-white' : 'bg-white text-blue-500'}`;
      btn.addEventListener("click", () => renderTransactorsPage(i));
      paginationContainer.appendChild(btn);
    }

    const nextBtn = document.createElement("button");
    nextBtn.textContent = "Next";
    nextBtn.disabled = currentPageTransactors === totalPagesTransactors;
    nextBtn.className = "px-3 py-1 border rounded bg-white text-blue-500 disabled:opacity-50";
    nextBtn.addEventListener("click", () => renderTransactorsPage(currentPageTransactors + 1));
    paginationContainer.appendChild(nextBtn);
  }

  renderTransactorsPage(1);

  // My Transactions Pagination
  const rowsTransactions = document.querySelectorAll("#myTransactionsTableBody tr");
  let currentPageTransactions = 1;
  const totalPagesTransactions = Math.ceil(rowsTransactions.length / perPage);
  const paginationTransactionsContainer = document.getElementById("myTransactionsPagination");
  ...
  function renderTransactionsPagination() {
    paginationTransactionsContainer.innerHTML = "";
    const prevBtn = document.createElement("button");
    prevBtn.textContent = "Prev";
    prevBtn.disabled = currentPageTransactions === 1;
    prevBtn.className = "px-3 py-1 border rounded bg-white text-blue-500 disabled:opacity-50";
    prevBtn.addEventListener("click", () => renderTransactionsPage(currentPageTransactions - 1));
    paginationTransactionsContainer.appendChild(prevBtn);

    for (let i = 1; i <= totalPagesTransactions; i++) {
      const btn = document.createElement("button");
      btn.textContent = i;
      btn.className = `px-3 py-1 border rounded ${i === currentPageTransactions ? 'bg-blue-500 text-white' : 'bg-white text-blue-500'}`;
      btn.addEventListener("click", () => renderTransactionsPage(i));
      paginationTransactionsContainer.appendChild(btn);
    }

    const nextBtn = document.createElement("button");
    nextBtn.textContent = "Next";
    nextBtn.disabled = currentPageTransactions === totalPagesTransactions;
    nextBtn.className = "px-3 py-1 border rounded bg-white text-blue-500 disabled:opacity-50";
    nextBtn.addEventListener("click", () => renderTransactionsPage(currentPageTransactions + 1));
    paginationTransactionsContainer.appendChild(nextBtn);
  }
