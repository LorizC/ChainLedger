  ...
  <div class="pc-container">
    <div class="pc-content">
      ...
      <div class="flex flex-col md:flex-row gap-6 px-6 items-start">
        ...
        <!-- LEFT: Profile Card -->
        <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow w-full md:w-[600px] flex-shrink-0"
          x-data="{
            open:false,
          ...
          <div class="flex items-center space-x-6">
            ...
            <div class="max-w-[250px]">
              <h2 class="text-4xl font-extrabold text-indigo-700 dark:text-white truncate" x-text="username" :title="username"></h2>
              <p class="text-xl text-gray-500 dark:text-gray-300 mt-1 truncate" x-text="fullname" :title="fullname"></p>
            </div>
          </div>
          ...
          <div x-show="open" x-cloak class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
            <div class="bg-white rounded-xl shadow-lg p-6 w-[28rem] dark:bg-gray-800">
              ...
              <form method="POST" action="profile.php">
                ...
                <input type="text" name="username" x-model="username" class="w-full border rounded-lg px-3 py-3 mb-4 text-lg text-gray-700 focus:ring-2 focus:ring-indigo-500">

                <div class="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    <label class="block text-sm font-medium text-gray-600 mb-1 dark:text-gray-300">First Name</label>
                    <input type="text" name="first_name" value="<?= htmlspecialchars($user['first_name']) ?>" class="w-full border rounded-lg px-3 py-3 text-lg text-gray-700 focus:ring-2 focus:ring-indigo-500">
                  </div>
                  <div>
                    <label class="block text-sm font-medium text-gray-600 mb-1 dark:text-gray-300">Last Name</label>
                    <input type="text" name="last_name" value="<?= htmlspecialchars($user['last_name']) ?>" class="w-full border rounded-lg px-3 py-3 text-lg text-gray-700 focus:ring-2 focus:ring-indigo-500">
                  </div>
                </div>
                ...
                <div class="grid grid-cols-5 gap-4 mb-6">
                  <template x-for="a in [
                    ...
                    <img :src="a" @click="avatar=a" class="w-16 h-16 rounded-full cursor-pointer border hover:ring-4 hover:ring-indigo-500 transition" :class="{'ring-4 ring-indigo-600':avatar===a}">
                  </template>
                </div>
                ...
                <div class="flex justify-end space-x-3">
                  <button type="button" @click="open=false" class="px-5 py-2 text-base text-gray-600 hover:underline dark:text-gray-300">Cancel</button>
                  <button type="submit" class="px-5 py-2 text-base bg-indigo-600 text-white rounded-lg dark:bg-slate-100 dark:text-gray-800">Save</button>
                </div>
              </form>
            </div>
          </div>
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow flex-1 w-full md:max-w-[600px] md:flex-shrink-0 min-h-[300px]">
          <div class="space-y-6 text-lg mt-8">
            <div class="flex justify-between">
              ...
              <span class="font-bold text-gray-800 dark:text-white max-w-[150px] truncate text-right"><?= htmlspecialchars($user["first_name"]) ?></span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-500 dark:text-gray-300 font-medium">Last Name</span>
              <span class="font-bold text-gray-800 dark:text-white max-w-[150px] truncate text-right"><?= htmlspecialchars($user["last_name"]) ?></span>
            </div>
            <div class="flex justify-between">
