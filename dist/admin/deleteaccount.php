  ...
  <div class="pc-container">
    <div class="pc-content">
      ...
      <?php if (isset($_SESSION['redirect_after_delete']) && $_SESSION['redirect_after_delete']): ?>
        ...
        <?php unset($_SESSION['redirect_after_delete']); ?>
      <?php endif; ?>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 px-6">
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow">
          <div class="flex items-center mb-6">
            ...
            <div class="ml-6 max-w-[220px]">
              <h2 class="text-3xl font-bold text-indigo-700 dark:text-white truncate" 
                  title="<?= htmlspecialchars($_SESSION['user']['username']); ?>">
                <?= htmlspecialchars($_SESSION['user']['username']); ?>
              </h2>
              <p class="text-lg text-gray-500 dark:text-gray-300 mt-1 truncate" 
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow flex-1"
             x-data="{
               ...
               submitForm() {
                 if (this.validate()) this.confirmModal = true;
               },
               confirmDelete() {
      ...
      if (deleteForm) {
        deleteForm.addEventListener('keypress', function (e) {
          if (e.key === 'Enter') {
            ...
            if (alpineRoot && alpineRoot.__x) alpineRoot.__x.$data.confirmModal = true;
          }
        });
        deleteForm.addEventListener('submit', function (e) {
          const alpineRoot = deleteForm.closest('[x-data]');
          if (alpineRoot && alpineRoot.__x && !alpineRoot.__x.$data.confirmModal) e.preventDefault();
        });
      }
    });
  </script>
